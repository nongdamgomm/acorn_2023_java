 ...
 *	 입력, 출력 (Input, Output)
 *	
 *	- 어떤 대상으로부터 데이터를 메모리로 읽어들이는 것을 입력 이라고 한다
 *	- 프로그래밍 언어의 관점에서 메모리 라는 것은 변수 or 필드 or 객체로 생각하면 편하다
 *	- 데이터는 2진수로 이루어져 있지만 2진수 8개의 묶음인 1byte 단위로 생각하면 편하다
 *  - 1byte는 총 256 가지의 값을 표현할 수 있다.
 *  - 1byte를 10 진수로 환산하면 0~255 가지의 숫자 중에 하나이다.
